...
- Get a launcher next to the video first: nothing in the pipeline puts one there. Copy the one for the display machine out of `build/launchers/` to wherever the video will be played from, and copy `build/playback.txt` (`loop` or `once`, written by `bin/build`) beside it. Then test it by double-clicking it on that machine. What each one does differs. `start-slideshow.command` on a Mac opens a terminal that must stay open, kills any running player, and plays the file fullscreen under `caffeinate`, looking beside itself, then in the project's `build/`, then on any mounted USB stick. `start-slideshow-usb.command` holds the Mac awake the same way but looks only in its own folder, which is the stick it sits on. The Windows `start-slideshow-usb.bat` also plays only what is in its own folder, and holds nothing awake: turn screen sleep off in Windows power settings yourself, as its own comments say. There is no plain Windows edition, so the USB one is the Windows VLC launcher wherever you put it, and no Windows VLC launcher ever looks in `..\build` or on another stick; the Windows browser launcher is the one that does look in `..\build`, for `player.html`. None of them mutes: a silent file plays nothing, a file with music plays it, and the venue's volume is set by hand. In each place it searches, a launcher takes the first `slideshow-x*.mp4` in name order (the `--concat` output, `slideshow-x3.mp4` with the default copies), then `slideshow.mp4` (the single loop); a `-labels` test render and the `-silent` copy of a render with music are never picked. Keep one of those two names on the stick and delete stale copies, or the wrong one plays. The two `.command` launchers look for `playback.txt` beside the file they are about to play, then in the script's own folder, then in `../build`, and the Windows `.bat` in its own folder, which is where it plays the file from; each passes `--repeat` only when the marker says `loop`. Without it they repeat, as they always have - which is wrong for a show that ends, so a `once` show that is missing its marker plays its end card and starts again.
- A launcher that came out of a downloaded zip carries the operating system's mark on it, and the first double-click is refused. On a Mac, Gatekeeper says the file is from an unidentified developer or cannot be verified; open it once by right-clicking it and choosing Open, or allow it in System Settings > Privacy & Security, and the double-click works from then on. On Windows, SmartScreen puts up "Windows protected your PC" for a downloaded `.bat`; choose More info, then Run anyway. A copy that arrived on a USB stick or came from a git clone carries no such mark. Clear this on the display machine at the test, not in front of the room.
...
2. If the file is missing, the Mac's `start-slideshow.command` looks in the project's `build/` and then on any mounted USB stick; both USB editions play only what sits in their own folder, so the spare copy has to be beside the launcher.
