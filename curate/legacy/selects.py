...
D=os.path.expanduser("~/photo-project/slideshow-all"); S=os.path.join(D,"_selects")
BUDGET=float(sys.argv[1]) if len(sys.argv)>1 else 145.0
start=time.time(); os.makedirs(S,exist_ok=True)
rows=[r for r in csv.DictReader(open(os.path.join(D,"recovered-dates.csv"),encoding="utf-8"))
      if r["selected"]=="yes"]
have={n:os.path.getsize(os.path.join(S,n)) for n in os.listdir(S) if os.path.isfile(os.path.join(S,n))}
todo=[r for r in rows if have.get(r["filename"]) != os.path.getsize(os.path.join(D,r["filename"]))]
print("selected %d | already copied %d | todo %d"%(len(rows),len(rows)-len(todo),len(todo)),flush=True)
stop=False
def cp(r):
    global stop
    if stop or time.time()-start>BUDGET: stop=True; return
    try: shutil.copy2(os.path.join(D,r["filename"]),os.path.join(S,r["filename"]))
    except Exception as e: return (r["filename"],str(e)[:60])
with ThreadPoolExecutor(max_workers=12) as ex: errs=[x for x in ex.map(cp,todo) if x]
n=len([1 for x in os.listdir(S) if os.path.isfile(os.path.join(S,x))])
print("ELAPSED %.0fs  in _selects: %d/%d  errors %d"%(time.time()-start,n,len(rows),len(errs)),flush=True)
