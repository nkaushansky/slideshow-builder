...
import os, sys, csv, json, time, subprocess, tempfile, collections, warnings
...
CS=os.path.join(D,"_contact-sheets"); os.makedirs(CS,exist_ok=True)
...
for y in sorted(byy):
    ...
    Wd=COLS*(TH+PAD)+PAD; Ht=sum(max(t[1].height for t in rr)+LAB+PAD for rr in R)+PAD+50
    sh=Image.new("RGB",(Wd,Ht),(250,250,250)); d=ImageDraw.Draw(sh)
    nv=sum(1 for _,_,v in ims if v)
    d.text((PAD,12),"%s  —  %s %s"%(y,HONOREE,AGE.get(int(y),"")),font=fh,fill=(15,15,15))
    d.text((PAD,34),"%d files in the cut  (%d video)"%(len(L),nv),font=fb,fill=(110,110,110))
    yy=50+PAD
    for rr in R:
        rh=max(t[1].height for t in rr); x=PAD
        for r,im,isv in rr:
            sh.paste(im,(x,yy))
            col=(170,70,30) if isv else (30,110,65)
            d.rectangle([x-2,yy-2,x+im.width+1,yy+im.height+1],outline=col,width=2)
            lab=r["best_guess_date"]
            if r["precision"]=="month": lab+="  (month)"
            elif r["precision"]=="year": lab+="  (year only)"
            d.text((x,yy+im.height+3),("VIDEO  " if isv else "")+lab,font=fb,fill=col)
            d.text((x,yy+im.height+15),r["filename"][11:][:34],font=f1,fill=(105,105,105))
            x+=TH+PAD
        yy+=rh+LAB+PAD
    sh.save(out,"JPEG",quality=86)
    print("wrote %s  %dx%d  (%d files)"%(os.path.basename(out),sh.size[0],sh.size[1],len(L)),flush=True)
