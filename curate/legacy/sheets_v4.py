...
import json as _j
ANCH=_j.load(open(os.path.expanduser("~/slideshow-work/v4_tags.json")))
...
for y in sorted(byy):
    ...
    def ncuts(r):
        c=0
        for col,rk in (("selected","shortlist_rank"),("selected_v2","v2_rank"),("selected_v4","__skip")):
            pass
        ...
        return c
    shared=sum(1 for r in L if ncuts(r)>0)
    d.text((PAD,12),"%s  (v4 - the synthesis)  -  %s %s"%(y,HONOREE,AGE.get(int(y),"")),font=fh,fill=(15,15,15))
    d.text((PAD,34),"%d files - %d appear in an earlier cut  -  x3/x2/x1 = how many of v1-v3 chose it, tags mark anchors"%(len(L),shared),font=fb,fill=(110,110,110))
    yy=52+PAD
    for rr in R:
