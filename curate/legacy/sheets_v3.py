...
V=os.path.expanduser("~/photo-project/slideshow-all v3")
...
rows=[r for r in csv.DictReader(open(os.path.join(D,"recovered-dates.csv"),encoding="utf-8"))
      if r.get("selected_v3")=="yes"]
import json as _j
ANCH=_j.load(open(os.path.expanduser("~/slideshow-work/v3_anchors.json")))
...
for y in sorted(byy):
    ...
    shared=sum(1 for r in L if r["selected"]=="yes" or r["selected_v2"]=="yes")
    d.text((PAD,12),"%s  (v3 - breadth + traditions)  -  %s %s"%(y,HONOREE,AGE.get(int(y),"")),font=fh,fill=(15,15,15))
    d.text((PAD,34),"%d files - %d appear in v1 or v2  -  * = also in an earlier cut, tags mark tradition anchors"%(len(L),shared),font=fb,fill=(110,110,110))
    yy=52+PAD
    for rr in R:
        ...
        for r,im,isv in rr:
            ...
            tag="* " if (r["selected"]=="yes" or r["selected_v2"]=="yes") else ""
            a=ANCH.get(r["filename"])
            if a: tag+="["+a+"] "
